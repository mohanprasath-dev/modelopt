...
export function ModelRecommendationCard({ data, index }: ModelRecommendationCardProps) {
  return (
    ...
    >
      ...
      <h3 className="text-xl font-semibold text-slate-100">{data.display_name}</h3>
      <p className="mt-2 text-sm leading-relaxed text-slate-300">{data.reasoning}</p>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2 text-xs">
        ...
        <div className="rounded-lg border border-slate-700 bg-slate-950/80 p-2">
          ...
          <p className="mt-1 text-slate-100">{data.recommended_quantization}</p>
        </div>
      </div>
      ...
      <div className="mt-4 grid gap-2 text-xs sm:grid-cols-2">
        <div className="rounded-lg border border-slate-700 bg-slate-950/70 p-2">
          ...
          <p className="text-slate-300">{data.strengths}</p>
        </div>
        <div className="rounded-lg border border-slate-700 bg-slate-950/70 p-2">
          <p className="mb-1 font-medium text-amber-300">Weaknesses</p>
          <p className="text-slate-300">{data.weaknesses}</p>
        </div>
      </div>
    </motion.article>
  )
